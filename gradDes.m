  ...
  if num_active_rare>=2
    ...
    if num_active_rare>=2
      ...
      for i=1:length(tempR)-1
        L2 = L2+norm([ParamR_old(tempR(i)).beta0 ParamR_old(tempR(i)).beta])^2;
      end
    end 
...
if num_active_normal>=2
    ...
    for i=1:length(tempN)-1
        L2 = L2+norm([ParamN_old(tempN(i)).beta0 ParamN_old(tempN(i)).beta])^2;
    end
end
...
while abs(Dnew-Dold)>=reltol||Dnew>Dold||isinf(Dold) 
    ...
    ObjF_meta(Fs,Fs_multiclassN,Fs_multiclassR,L2,data_GTT_tempN,data_GTT_tempR,...
	      label_tempN,label_tempR,label,active_set_normal,active_set_rare,WN,WR,a_u,mode,rare_exist);
