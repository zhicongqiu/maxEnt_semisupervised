function [error FA FN error_CN error_CR error_avgC]= calculate_error(test_PMFunknown,test_PMFnormal,test_PMFrare,test_GTT,count)
...
if count == 0 %report error rate on test set
    ...
    error_avgC = mean([error_normalPC(total_normalPC~=0)./total_normalPC(total_normalPC~=0) error_rarePC(total_rarePC~=0)./total_rarePC(total_rarePC~=0)]);
end
